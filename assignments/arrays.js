...
console.log(
  `The last car in the inventory is a ${inventory[lastCar].car_make} ${
...
// Alternate answer using .map()
console.log(inventory.map(x => x.car_model).sort());
...
// Alternate answer using .map()
console.log(inventory.map(x => x.car_year));
...
// Alternate answer using .filter()
console.log(inventory.filter(x => x.car_year < 2000).length);
...
console.log(
  JSON.stringify(
    inventory.filter(x => x.car_make === "BMW" || x.car_make === "Audi")
  )
);
